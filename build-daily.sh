...
echo 
echo "/######################################################################/"
echo " Build new image terrabrasilis/amazon-alert-daily:${TAG_VERSION} "
echo "/######################################################################/"
echo
docker build $NO_CACHE -t "terrabrasilis/amazon-alert-daily:${TAG_VERSION}" --build-arg VERSION=$VERSION --build-arg INDEX_FILE=deter-amazon-daily -f environment/Dockerfile .
...
if [[ ! "$1" = "silent" ]]; then
    ...
    if [[ ! "$SEND_TO_HUB" = "yes" ]]; then
        ...
        docker push "terrabrasilis/amazon-alert-daily:${TAG_VERSION}"
    fi
fi