...
URL="${BASE_URL}/wfs?SERVICE=WFS&REQUEST=GetFeature&VERSION=2.0.0&TYPENAME=all_daily_d&OUTPUTFORMAT=csv"
curl "$URL" -H 'User-Agent: Shell script with CURL' --compressed -H 'Pragma: no-cache' -H 'Cache-Control: no-cache' > deter-amazon-daily.csv

URL="${BASE_URL}/wfs?SERVICE=WFS&REQUEST=GetFeature&VERSION=2.0.0&TYPENAME=updated_date&OUTPUTFORMAT=application%2Fjson"
